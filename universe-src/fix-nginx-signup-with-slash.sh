#!/bin/bash
echo "=========================================="
echo "ИСПРАВЛЕНИЕ NGINX ДЛЯ /signup (с trailing slash)"
echo "=========================================="
echo ""
...
server {
    ...
    # КРИТИЧНО: Обрабатываем версию С trailing slash ПЕРВОЙ
    # Это предотвращает редирект 308
    ...
    # Обработка /signup БЕЗ trailing slash
    location = /api/datalab/auth/signup {
        if ($request_method = OPTIONS) {
            add_header Access-Control-Allow-Origin "*" always;
            add_header Access-Control-Allow-Methods "GET, POST, PUT, DELETE, PATCH, OPTIONS" always;
    ...
    # Обработка /register - точное совпадение
...
echo ""
echo "Проверка /signup (без slash):"
STATUS1=$(curl -s -o /dev/null -w "%{http_code}" -X OPTIONS https://api.civilx.ru/api/datalab/auth/signup -H "Origin: http://civilxuniverse.ru" -H "Access-Control-Request-Method: POST" -k)
echo "HTTP Status Code: $STATUS1"

echo ""
echo "Проверка /signup/ (с slash):"
STATUS2=$(curl -s -o /dev/null -w "%{http_code}" -X OPTIONS https://api.civilx.ru/api/datalab/auth/signup/ -H "Origin: http://civilxuniverse.ru" -H "Access-Control-Request-Method: POST" -k)
...
if [ "$STATUS1" = "204" ] || [ "$STATUS2" = "204" ]; then
    ...
    curl -s -X OPTIONS https://api.civilx.ru/api/datalab/auth/signup \
      -H "Origin: http://civilxuniverse.ru" \
      -H "Access-Control-Request-Method: POST" \
      -k -I | grep -i "access-control"
else
    echo "❌ Оба варианта не работают"

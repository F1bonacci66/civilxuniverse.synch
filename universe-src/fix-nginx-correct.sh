#!/bin/bash
# ИСПРАВЛЕННОЕ РЕШЕНИЕ: Без add_header в server блоке

echo "=========================================="
echo "ИСПРАВЛЕННОЕ РЕШЕНИЕ"
echo "=========================================="
echo ""
...
server {
    ...
    # Обрабатываем OPTIONS в location блоке
...
server {
    ...
    # КРИТИЧНО: Используем location ^~ для максимального приоритета
    ...
    location ^~ /api/datalab/auth/register {
        ...
        rewrite ^(/api/datalab/auth/register)/?$ /api/datalab/auth/register break;

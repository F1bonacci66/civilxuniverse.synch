#!/bin/bash
# Автоматическое исправление конфигурации Nginx для api.civilx.ru

echo "🔧 Исправление конфигурации Nginx..."

# Копируем исправленную конфигурацию
cp /tmp/api_nginx_fixed.conf /etc/nginx/sites-available/api.civilx.ru 2>/dev/null || {
    echo "⚠️  Файл /tmp/api_nginx_fixed.conf не найден. Создаю конфигурацию напрямую..."
...
if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        curl -k https://api.civilx.ru/api/datalab/health
        echo ""
        echo "✅ Готово!"
    else

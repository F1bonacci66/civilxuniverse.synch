#!/bin/bash
echo "=========================================="
echo "НАСТРОЙКА API НА civilxuniverse.ru"
echo "=========================================="
echo ""
...
server {
    ...
    # КРИТИЧНО: API endpoints - обрабатываем ПЕРЕД основным location /
...
echo ""
echo "Проверка /api/datalab/auth/signup/ через civilxuniverse.ru:"
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X OPTIONS http://civilxuniverse.ru/api/datalab/auth/signup/ -H "Origin: http://civilxuniverse.ru" -H "Access-Control-Request-Method: POST")
echo "HTTP Status Code: $STATUS"

if [ "$STATUS" = "204" ]; then
    echo "✅ УСПЕХ! API работает на civilxuniverse.ru!"
    echo ""
    echo "Проверка заголовков:"
    curl -s -X OPTIONS http://civilxuniverse.ru/api/datalab/auth/signup/ \
      -H "Origin: http://civilxuniverse.ru" \
      -H "Access-Control-Request-Method: POST" \
      -I | grep -i "access-control\|X-Location"
else
    echo "❌ Статус: $STATUS"
    echo ""
    echo "Проверка полного ответа:"
    curl -v -X OPTIONS http://civilxuniverse.ru/api/datalab/auth/signup/ \
      -H "Origin: http://civilxuniverse.ru" \
      -H "Access-Control-Request-Method: POST" \
      2>&1 | grep -E "^< HTTP|^< X-Location|^> OPTIONS"
fi

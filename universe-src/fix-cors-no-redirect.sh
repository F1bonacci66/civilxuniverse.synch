#!/bin/bash
# Исправление: предотвращаем редирект на trailing slash

echo "🔍 ПРОБЛЕМА НАЙДЕНА!"
echo "Nginx делает редирект с /api/datalab/auth/register на /api/datalab/auth/register/"
echo "Это происходит ДО обработки OPTIONS запроса."
echo ""
...
server {
    ...
    # Этот блок должен обработать запрос ДО того, как FastAPI сделает редирект
    location = /api/datalab/auth/register {
        ...
        # КРИТИЧНО: Используем proxy_pass БЕЗ trailing slash, чтобы избежать редиректа
        ...
        # КРИТИЧНО: Не следовать редиректам от backend
    ...
    # Обработка OPTIONS для register С trailing slash
    location = /api/datalab/auth/register/ {
        if ($request_method = OPTIONS) {
            add_header Access-Control-Allow-Origin "*" always;
            add_header Access-Control-Allow-Methods "GET, POST, PUT, DELETE, PATCH, OPTIONS" always;
    ...
    location ~ ^/api/datalab/ {
        ...
        # НЕ используем rewrite для удаления trailing slash, так как это может вызвать редирект

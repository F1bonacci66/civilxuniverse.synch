#!/bin/bash
# Финальное решение: проверяем, откуда идет редирект

echo "🔍 Проверка, откуда идет редирект 308..."
echo ""

echo "1. Проверка FastAPI - делает ли он редирект:"
...
echo "2. Проверка FastAPI с trailing slash:"
curl -v -X OPTIONS http://127.0.0.1:8000/api/datalab/auth/register/ \
  -H "Origin: http://civilxuniverse.ru" \
  -H "Access-Control-Request-Method: POST" \
  2>&1 | grep -E "< HTTP|< Location"
...
echo "3. Проверка, может быть проблема в том, что nginx добавляет trailing slash автоматически"
echo ""

echo "🔧 Применение финального решения..."
echo "Проблема: редирект происходит ДО обработки location блока."
echo "Решение: обрабатываем OPTIONS в самом начале, до proxy_pass"
echo ""

# Создаем конфигурацию, которая обрабатывает OPTIONS ДО proxy_pass
...
server {
    ...
    # КРИТИЧНО: Обрабатываем OPTIONS для register БЕЗ trailing slash
    ...
    location = /api/datalab/auth/register {
        ...
        # Для остальных методов - проксируем, но с явным путем БЕЗ trailing slash
    ...
    # Обрабатываем OPTIONS для register С trailing slash
    location = /api/datalab/auth/register/ {
        if ($request_method = OPTIONS) {
            add_header Access-Control-Allow-Origin "*" always;
            add_header Access-Control-Allow-Methods "GET, POST, PUT, DELETE, PATCH, OPTIONS" always;
...
if [ $? -eq 0 ]; then
    ...
    if [ "$STATUS" = "204" ]; then
        ...
        echo ""
        echo "Возможно, проблема в FastAPI. Проверьте настройку redirect_slashes в FastAPI."
    fi
else

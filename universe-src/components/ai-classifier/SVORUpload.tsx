...
export function SVORUpload({ onUploadComplete }: SVORUploadProps) {
  ...
  const parseCSV = (text: string, defaultGroupName?: string): ParseResult => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (!line || line === ',') {
        console.log(`[SVOR Parse] Строка ${i + 1}: пропущена (пустая)`)
        continue
      }
      ...
      // Валидация обязательных полей - пропускаем строки с пустыми значениями
      if (!code || code === '') {
        console.warn(`[SVOR Parse] Строка ${i + 1}: пропущена (пустой svor_code)`)
        errors.push(`Строка ${i + 1}: отсутствует svor_code`)
        continue
      }
      if (!name || name === '') {
        console.warn(`[SVOR Parse] Строка ${i + 1}: пропущена (пустой svor_name)`)
      ...
      if (descIndex !== -1 && values[descIndex]) {
        const desc = values[descIndex].replace(/^"|"$/g, '').trim()
        if (desc) {
          position.svorDescription = desc
        }
      }

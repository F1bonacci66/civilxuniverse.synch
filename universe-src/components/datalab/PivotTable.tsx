...
interface PivotTableProps {
  data: PivotResponse
  onLoadLevel?: (level: number, parentGroupKey: string, offset: number) => Promise<PivotResponse>
  pivotRequest?: {
    projectId?: string
    versionId?: string
    rows: string[]
...
export function PivotTable({ data, onLoadLevel, pivotRequest }: PivotTableProps) {
  ...
  // Состояние для отслеживания загруженных данных по уровням
  // Ключ: "level:groupKey", значение: { rows: string[], cells: any[], hasMore: boolean, offset: number }
  const [loadedLevels, setLoadedLevels] = useState<Map<string, { rows: string[], cells: any[], hasMore: boolean, offset: number }>>(new Map())
  
  // Состояние для отслеживания загрузки
  const [loadingLevels, setLoadingLevels] = useState<Set<string>>(new Set())
  
  // Объединенные данные (исходные + загруженные по уровням)
  const [mergedData, setMergedData] = useState<PivotResponse>(data)
  
  // Обновляем mergedData при изменении data
  useEffect(() => {
    setMergedData(data)
  }, [data])
  
  // Ограничиваем начальную загрузку до 500 строк на первый уровень
  useEffect(() => {
    if (!onLoadLevel || !pivotRequest || mergedData.rows.length <= 500) return
    
    // Если данных больше 500, ограничиваем отображение
    const limitedRows = mergedData.rows.slice(0, 500)
    const limitedCells = mergedData.cells.filter(cell => 
      limitedRows.includes(cell.rowKey)
    )
    
    if (limitedRows.length < mergedData.rows.length) {
      setMergedData(prev => ({
        ...prev,
        rows: limitedRows,
        cells: limitedCells
  ...
  }, [mergedData, onLoadLevel, pivotRequest])

  // Переключение состояния свернутости группы
  const toggleGroup = useCallback(async (level: number, groupKey: string) => {
    ...
    setCollapsedGroups((prev) => {
      ...
      if (wasCollapsed) {
        ...
        // Загружаем данные следующего уровня при раскрытии
        if (onLoadLevel && !loadedLevels.has(key)) {
          const nextLevel = level + 1
          setLoadingLevels(prev => new Set(prev).add(key))
          onLoadLevel(nextLevel, groupKey, 0)
            .then((response) => {
              ...
              setMergedData(prev => ({
                ...prev,
                rows: [...new Set([...prev.rows, ...(response.rows || [])])],
                cells: [...prev.cells, ...(response.cells || [])]
              }))
            })
            .catch((err) => {
  ...
  }, [onLoadLevel, loadedLevels])
  
  // Загрузка дополнительных данных для уровня
  const loadMoreForLevel = useCallback(async (level: number, groupKey: string) => {
    if (!onLoadLevel) return
    
    const key = `${level - 1}:${groupKey}`
    const levelData = loadedLevels.get(key)
    if (!levelData) return
    
    const nextOffset = levelData.offset + 500
    setLoadingLevels(prev => new Set(prev).add(key))
    
    try {
      const response = await onLoadLevel(level, groupKey, nextOffset)
      setLoadedLevels(prev => {
        const newMap = new Map(prev)
        const existing = newMap.get(key) || { rows: [], cells: [], hasMore: false, offset: 0 }
        newMap.set(key, {
          rows: [...new Set([...existing.rows, ...(response.rows || [])])],
          cells: [...existing.cells, ...(response.cells || [])],
          hasMore: (response as any).hasMore || false,
          offset: nextOffset
        })
        return newMap
  ...
  const currentRows = mergedData.rows || rows
  const currentCells = mergedData.cells || cells
  ...
  const hierarchicalRows = useMemo(() => {
    ...
    const parsedRows: HierarchicalRow[] = currentRows.map((row: any) => {
  ...
  const collapsedGroupAggregations = useMemo(() => {
    ...
    hierarchicalRows.forEach((row: HierarchicalRow, rowIndex: number) => {
      for (let level = 0; level < fieldsCount - 1; level++) {
        ...
        if (collapsedGroups.has(key)) {
          ...
          // ВАЖНО: parentRowKey должен быть строкой именно этого уровня (values.length === level + 1)
          // Если текущая строка является строкой этого уровня, используем её
          // Иначе ищем первую строку группы с нужным уровнем
          if (isFirstInGroup) {
            let parentRowKey = row.rowKey
            
            // Если текущая строка не является строкой этого уровня, ищем первую строку группы с нужным уровнем
            if (row.values.length !== level + 1) {
              // Ищем первую строку группы с values.length === level + 1
              const parentRow = hierarchicalRows.find((r: HierarchicalRow) => {
                const rGroupKey = r.values.slice(0, level + 1).join(' | ')
                return rGroupKey === groupKey && r.values.length === level + 1
              })
              
              if (parentRow) {
                parentRowKey = parentRow.rowKey
              } else {
                // Если не нашли строку нужного уровня, используем первую строку группы
                // (это может быть, если все строки имеют больший уровень вложенности)
    ...
    collapsedGroupsList.forEach(({ level, groupKey, parentRowKey }) => {
      ...
      // Находим все строки, которые принадлежат этой группе
      // Дочерние строки должны иметь:
      // 1. Тот же groupKey (первые level+1 значений совпадают)
      // 2. Уровень вложенности больше level (т.е. values.length > level + 1)
      // 3. rowKey !== parentRowKey (не является родительской строкой)
      ...
      // Родительская строка - это строка с values.length === level + 1 (строка именно этого уровня)
      // Дочерние строки - это все строки группы с уровнем вложенности > level, кроме родительской
      // ВАЖНО: включаем ВСЕ дочерние строки, включая первую
      
      // Ищем родительскую строку (строка с values.length === level + 1)
      const parentRow = allGroupRows.find((r: HierarchicalRow) => r.values.length === level + 1)
      
      // Определяем, является ли parentRowKey родительской строкой или дочерней
      const parentRowKeyRow = allGroupRows.find((r: HierarchicalRow) => r.rowKey === parentRowKey)
      const isParentRowKeyActuallyParent = parentRowKeyRow && parentRowKeyRow.values.length === level + 1
      
      const childRows = allGroupRows.filter((r: HierarchicalRow) => {
        // Исключаем только настоящую родительскую строку (values.length === level + 1)
        if (parentRow && r.rowKey === parentRow.rowKey) return false
        
        // Если parentRowKey указывает на родительскую строку, исключаем её
        if (isParentRowKeyActuallyParent && r.rowKey === parentRowKey) return false
        
        // Включаем только строки с уровнем вложенности больше level
        // (т.е. строки, которые являются дочерними для этой группы)
        // ВАЖНО: включаем ВСЕ дочерние строки, включая первую
        return r.values.length > level + 1
      })
      ...
      console.log('📊 PivotTable: Строки группы', {
        ...
        parentRow: parentRow ? {
          ...
          valuesLength: parentRow.values.length
        } : null,
        parentRowKeyRow: parentRowKeyRow ? {
          rowKey: parentRowKeyRow.rowKey,
          values: parentRowKeyRow.values,
          level: parentRowKeyRow.level,
          valuesLength: parentRowKeyRow.values.length,
          isActuallyParent: isParentRowKeyActuallyParent
        } : null,
        childRowsCount: childRows.length,
        childRowKeys: childRows.map((r: HierarchicalRow) => ({
          rowKey: r.rowKey,
          values: r.values,
          level: r.level,
          valuesLength: r.values.length
        })),
        allGroupRowKeys: allGroupRows.map((r: HierarchicalRow) => ({
          rowKey: r.rowKey,
          valuesLength: r.values.length
        }))
      })
      ...
      colsToProcess.forEach((col: string) => {
        aggregationKeys.forEach((aggKey: string) => {
          ...
          // Добавляем значения ТОЛЬКО дочерних строк (не включаем родительскую)
          // ВАЖНО: обрабатываем все дочерние строки, включая первую
          childRows.forEach((childRow: HierarchicalRow, childIndex: number) => {
            ...
            console.log('📊 PivotTable: Обработка дочерней строки', {
              childIndex,
    ...
    <div className="overflow-x-auto">
      <table className="w-full border-collapse bg-[rgba(0,0,0,0.4)] rounded-lg overflow-hidden">
        ...
        <tbody>
          ...
          {/* Кнопка "Загрузить еще 500" для каждого уровня */}
          {onLoadLevel && actualRowsFields && actualRowsFields.length > 1 && (() => {
            const levelsToShow: Array<{ level: number, groupKey: string, hasMore: boolean, isLoading: boolean }> = []
            
            // Проверяем каждый уровень на наличие данных для загрузки
            for (let level = 0; level < actualRowsFields.length - 1; level++) {
              // Находим все группы на этом уровне
              const groupsAtLevel = new Set<string>()
              visibleRows.forEach((row: HierarchicalRow) => {
                if (row.values.length > level) {
                  const groupKey = row.values.slice(0, level + 1).join(' | ')
                  groupsAtLevel.add(groupKey)
                }
              })
              
              // Проверяем каждую группу
              groupsAtLevel.forEach((groupKey) => {
                const key = `${level}:${groupKey}`
                const levelData = loadedLevels.get(key)
                const isLoading = loadingLevels.has(key)
                
                // Показываем кнопку, если:
                // 1. Группа развернута (не свернута)
                // 2. Есть еще данные для загрузки
                if (!collapsedGroups.has(key) && (levelData?.hasMore || (!levelData && (data as any).hasMore))) {
                  levelsToShow.push({
                    level: level + 1,
                    groupKey,
                    hasMore: levelData?.hasMore || (data as any).hasMore || false,
                    isLoading
                  })
                }
              })
            ...
            return levelsToShow.map(({ level, groupKey, hasMore, isLoading }) => (
              <tr key={`load-more-${level}-${groupKey}`} className="bg-[rgba(0,0,0,0.3)]">
                <td 
                  colSpan={
                    (actualRowsFields?.length || 1) + 
                    (columns.length > 0 ? columns.length * aggregationKeys.length : aggregationKeys.length)
                  }
                  className="px-4 py-3 text-center"
                >
                  <button
                    onClick={() => loadMoreForLevel(level, groupKey)}
                    disabled={isLoading}
                  ...
                  >
                    {isLoading ? 'Загрузка...' : 'Загрузить еще 500 значений'}
                  </button>
                  {hasMore && (

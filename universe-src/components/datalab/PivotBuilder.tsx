...
export function PivotBuilder({
  ...
  // Кэш значений фильтров: ключ = `${field}:${JSON.stringify(filters)}:${JSON.stringify(selectedParameters)}`
  const filterValuesCacheRef = useRef<Record<string, string[]>>({})
  
  // Ref для debounce таймера каскадной перезагрузки
  const cascadeReloadTimerRef = useRef<NodeJS.Timeout | null>(null)
  
  // Очистка кэша и сброс загруженных значений при изменении projectId, versionId или selectedParameters
  useEffect(() => {
    // Очищаем кэш при изменении базовых параметров
    filterValuesCacheRef.current = {}
    // Сбрасываем загруженные значения фильтров - они будут загружены заново при открытии
    setFilterValues({})
    setOpenFilterField(null) // Закрываем открытые фильтры
    console.log('🗑️ Кэш значений фильтров очищен, загруженные значения сброшены')
  }, [projectId, versionId, selectedParameters])
  ...
  // Сортируем параметры: выбранные вверху, затем остальные
  const sortedParameters = [...filteredParameters].sort((a, b) => {
    const aSelected = selectedParameters.includes(a)
    const bSelected = selectedParameters.includes(b)
  ...
  // Функция для загрузки значений фильтра с учетом других фильтров (с кэшированием)
  const loadFilterValues = async (field: string, currentFilters?: Record<string, string[]>, forceReload = false) => {
    ...
    // Создаем ключ кэша на основе поля, фильтров и выбранных параметров
    const cacheKey = `${field}:${JSON.stringify(filtersForRequest)}:${JSON.stringify(selectedParameters)}`
    
    // Проверяем кэш, если не требуется принудительная перезагрузка
    if (!forceReload && filterValuesCacheRef.current[cacheKey]) {
      console.log(`💾 Использованы кэшированные значения для поля "${field}"`)
      setFilterValues(prev => ({ ...prev, [field]: filterValuesCacheRef.current[cacheKey] }))
      return
    }
    ...
    try {
      ...
      // Сохраняем в кэш
      filterValuesCacheRef.current[cacheKey] = values
      
      // Очищаем старые записи кэша (оставляем только последние 50)
      const cacheKeys = Object.keys(filterValuesCacheRef.current)
      if (cacheKeys.length > 50) {
        const keysToDelete = cacheKeys.slice(0, cacheKeys.length - 50)
        keysToDelete.forEach(key => delete filterValuesCacheRef.current[key])
      }
  ...
  const handleFilterToggle = async (field: string) => {
    if (openFilterField === field) {
      ...
      // Загружаем значения только при открытии (кэш проверится внутри функции)
      // Если значения уже загружены и фильтры не изменились, используем кэш
      await loadFilterValues(field, undefined, false)
    }
  }
  ...
  // С debounce и параллельной загрузкой
  useEffect(() => {
    ...
    if (cascadeReloadTimerRef.current) {
      clearTimeout(cascadeReloadTimerRef.current)
    }
    ...
    const reloadFilterValues = async () => {
      ...
      // 1. Поля, для которых фильтр открыт (openFilterField) - приоритет
      // 2. Поля, для которых уже загружены значения (filterValues)
      const fieldsWithLoadedValues = Object.keys(filterValues)
      const fieldsToReload = Array.from(new Set([
        ...(openFilterField ? [openFilterField] : []),
        ...fieldsWithLoadedValues
      ]))
      ...
      // Параллельно перезагружаем значения для всех полей с актуальными фильтрами
      const reloadPromises = fieldsToReload
        .filter(field => {
          // Пропускаем, если поле не является фильтруемым
          return field !== 'selected_parameters' && !field.includes('selected_parameters')
        })
        .map(field => {
          ...
          return loadFilterValues(field, filters, true) // forceReload = true для каскадной фильтрации
        })
    ...
    // Запускаем перезагрузку с debounce (500мс) только если есть необходимые параметры
    if (projectId && versionId) {
      cascadeReloadTimerRef.current = setTimeout(() => {
        reloadFilterValues()
      }, 500) // Debounce 500мс
    }
  ...
  }, [filters, projectId, versionId, selectedParameters, openFilterField])
      ...
      <div className="bg-[rgba(0,0,0,0.4)] rounded-lg p-4 border border-[rgba(255,255,255,0.1)]">
        ...
        {selectedParameters.length > 0 && (
          <div className="mt-3 text-sm text-primary-400">
            ...
            {parameterSearch && sortedParameters.length > 0 && (
              <span className="text-[#999] ml-2">
                (в отфильтрованных: {sortedParameters.filter(p => selectedParameters.includes(p)).length})
              </span>
            )}
          </div>
        )}

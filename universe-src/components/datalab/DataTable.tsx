...
import { getCSVData, getDistinctValues, type CSVDataRow, type CSVDataFilters } from '@/lib/api/data'
...
export function DataTable({ fileUploadId, projectId, versionId, userId }: DataTableProps) {
  ...
  // Кэш уникальных значений для каждого столбца
  const [distinctValuesCache, setDistinctValuesCache] = useState<Record<ColumnFilterField, string[]>>({
    model_name: [],
    element_id: [],
    category: [],
    parameter_name: [],
  ...
  // Флаги загрузки уникальных значений
  const [loadingDistinctValues, setLoadingDistinctValues] = useState<Record<ColumnFilterField, boolean>>({
    model_name: false,
    element_id: false,
    category: false,
  ...
  const loadData = async (resetOffset = false) => {
    try {
      ...
      // Преобразуем фильтры по столбцам в массивы для API
      ...
      // Добавляем фильтры по столбцам (множественный выбор)
      // Преобразуем "(пусто)" обратно в пустую строку для API
      if (columnFilters.model_name.size > 0) {
        filters.modelName = Array.from(columnFilters.model_name).map(v => v === '(пусто)' ? '' : v)
      }
      if (columnFilters.element_id.size > 0) {
        filters.elementId = Array.from(columnFilters.element_id).map(v => v === '(пусто)' ? '' : v)
      }
      if (columnFilters.category.size > 0) {
        filters.category = Array.from(columnFilters.category).map(v => v === '(пусто)' ? '' : v)
      }
      if (columnFilters.parameter_name.size > 0) {
        filters.parameterName = Array.from(columnFilters.parameter_name).map(v => v === '(пусто)' ? '' : v)
      }
      if (columnFilters.parameter_value.size > 0) {
        filters.parameterValue = Array.from(columnFilters.parameter_value).map(v => v === '(пусто)' ? '' : v)
      }
  ...
  }, [search, categoryFilter, parameterFilter, sortBy, sortOrder, fileUploadId, projectId, versionId, columnFilters])
  ...
  // Загрузка уникальных значений для столбца из API (все данные)
  const loadDistinctValues = async (field: ColumnFilterField) => {
    // Если уже загружены, не загружаем повторно
    if (distinctValuesCache[field].length > 0) {
      return
    }

    // Если уже загружается, не запускаем повторно
    if (loadingDistinctValues[field]) {
      return
    }

    try {
      setLoadingDistinctValues((prev) => ({ ...prev, [field]: true }))
      ...
      // Преобразуем пустые значения в "(пусто)" для отображения
      const processedValues = values.map(v => (v === null || v === undefined || v === '') ? '(пусто)' : v)
      const sortedValues = processedValues.sort()
      
      setDistinctValuesCache((prev) => ({
        ...prev,
        [field]: sortedValues,
      }))
    } catch (err) {
      console.error(`Error loading distinct values for ${field}:`, err)
    } finally {
      setLoadingDistinctValues((prev) => ({ ...prev, [field]: false }))
    }
  }

  // Получение уникальных значений для столбца (из кэша или загрузка)
  const getUniqueValuesForColumn = (field: ColumnFilterField): string[] => {
    // Загружаем значения при первом обращении
    if (distinctValuesCache[field].length === 0 && !loadingDistinctValues[field]) {
      loadDistinctValues(field)
    }
    return distinctValuesCache[field]
  }

  // Фильтрация теперь происходит на сервере, используем данные как есть
  ...
  // Загрузка уникальных значений при открытии выпадающего списка
  useEffect(() => {
    if (openFilterDropdown) {
      const field = openFilterDropdown
      const allValues = distinctValuesCache[field]
      const isLoading = loadingDistinctValues[field]
      if (allValues.length === 0 && !isLoading) {
        loadDistinctValues(field)
      }
    }
  }, [openFilterDropdown])

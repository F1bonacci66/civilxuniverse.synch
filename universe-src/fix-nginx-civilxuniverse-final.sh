#!/bin/bash
echo "=========================================="
echo "ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ NGINX ДЛЯ civilxuniverse.ru"
echo "=========================================="
echo ""
...
server {
    ...
    # location ^~ имеет приоритет выше, чем location /, и останавливает дальнейший поиск
...
if [ "$STATUS2" = "422" ] || [ "$STATUS2" = "201" ]; then
    ...
    curl -v -X POST http://127.0.0.1/api/datalab/auth/signup/ \
      ...
      2>&1 | grep -E "^< HTTP|^< X-Location|^< x-powered"
fi

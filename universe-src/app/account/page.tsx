...
export default function AccountPage() {
  ...
  const loadAvailableVersions = async () => {
    ...
    try {
      const client = getApiClient()
      const url = `${client.baseURL}/product-versions/available`
      console.log('Загрузка версий, URL:', url)
      ...
      if (!response.ok) {
        ...
        throw new Error(`Ошибка загрузки версий: ${response.status} ${response.statusText}`)
      }
      ...
      if (data.success) {
        if (data.versions && data.versions.length > 0) {
          console.log('Найдено версий:', data.versions.length)
          setAvailableVersions(data.versions)
          // Получаем уникальные продукты
          const uniqueProducts: string[] = [...new Set(data.versions.map((v: any) => v.product_name as string))].sort() as string[]
          console.log('Уникальные продукты:', uniqueProducts)
          setProducts(uniqueProducts)
        } else {
          console.warn('Версии не найдены (пустой массив)')
          setAvailableVersions([])
          setProducts([])
        }
      } else {
        console.warn('API вернул success=false:', data)
        setProducts([])
      }
    } catch (err: any) {
      console.error('Ошибка загрузки версий:', err)
      setError(`Ошибка загрузки версий: ${err.message}`)
    } finally {
  ...
  const handleProductChange = (productName: string) => {
    ...
    if (productName) {
      const versions = availableVersions.filter(v => v.product_name === productName)
      const uniqueProductVersions: string[] = [...new Set(versions.map((v: any) => v.product_version as string))].sort() as string[]
      setProductVersions(uniqueProductVersions)
    } else {
      setProductVersions([])
    }
  }

  // Обработка выбора версии продукта
  const handleProductVersionChange = (productVersion: string) => {
    setSelectedProductVersion(productVersion)
  }

  // Скачивание файла
  const handleDownload = async () => {
    if (!selectedProduct || !selectedProductVersion) {
      setError('Выберите продукт и версию продукта')
      return
    }
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        product_version: selectedProductVersion,
      })
      
      const downloadUrl = `${client.baseURL}/product-versions/download?${params.toString()}`
      
      // Скачиваем файл через fetch с авторизацией
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ detail: 'Ошибка скачивания файла' }))
        throw new Error(errorData.detail || 'Ошибка скачивания файла')
      }

      // Получаем имя файла из заголовка Content-Disposition или формируем сами
      const contentDisposition = response.headers.get('Content-Disposition')
      let filename = `CivilX-${selectedProduct}-v${selectedProductVersion}.msi`
      ...
      setSuccess('Скачивание началось!')
        ...
        <div className="bg-[rgba(31,41,55,0.6)] backdrop-blur-[10px] border border-[rgba(255,255,255,0.1)] rounded-2xl p-8 mb-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div className="flex-1">
              <h2 className="text-2xl font-semibold text-white">Скачивание версий</h2>
              <p className="text-sm text-[#999] mt-1">Выберите продукт и версию продукта для скачивания</p>
            </div>
          </div>

          {loadingVersions ? (
            <div className="text-center py-8 text-white">Загрузка версий...</div>
          ) : (
            <div className="bg-[rgba(255,255,255,0.05)] rounded-xl p-6 border border-[rgba(255,255,255,0.1)]">
              <div className="flex flex-col md:flex-row gap-4 items-end">
                <div className="flex-1">
                  <label className="block text-sm font-medium text-white mb-2">Продукт</label>
                  <select
                  ...
                  >
                    ...
                    {products.map((product) => (
                      <option key={product} value={product} className="bg-[#2a2a2a] text-white">
                        {product}
                      </option>
                    ))}
                  </select>
                ...
                <div className="flex-1">
                  <label className="block text-sm font-medium text-white mb-2">Версия продукта</label>
                  <select
                    value={selectedProductVersion}
                    onChange={(e) => handleProductVersionChange(e.target.value)}
                    disabled={!selectedProduct}
                    className="w-full px-4 py-3 bg-[rgba(17,24,39,0.8)] border-2 border-[rgba(255,255,255,0.1)] rounded-xl text-white focus:outline-none focus:border-primary-500 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <option value="">Выберите версию продукта</option>
                    {productVersions.map((productVersion) => (
                      <option key={productVersion} value={productVersion} className="bg-[#2a2a2a] text-white">
                        {productVersion}
                      </option>
                    ))}
                  </select>
                ...
                <button
                  ...
                  disabled={!selectedProduct || !selectedProductVersion}
                  className="px-6 py-3 bg-gradient-to-r from-[#667eea] to-[#764ba2] text-white font-semibold rounded-xl hover:opacity-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 whitespace-nowrap"
                >
                  <span>⬇️</span>
                  <span>Скачать MSI</span>
                </button>
              </div>
            </div>
          )}

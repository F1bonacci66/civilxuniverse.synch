...
import { createPivotTable, getAvailableFields, updatePivotReport, getPivotReport } from '@/lib/api/pivot'
...
export default function VersionPivotPage({
  ...
  // Refs для сохранения актуальных значений при beforeunload
  const currentStateRef = useRef({
    activeTab: null as PivotReport | null,
    selectedParameters: [] as string[],
    rows: [] as string[],
    columns: [] as string[],
    values: [] as PivotAggregation[],
    filters: {} as Record<string, string[]>,
    pivotData: null as PivotResponse | null,
  })
  ...
  }, [activeTab, selectedParameters, rows, columns, values, filters, pivotData])
  
  // Функция для сохранения текущей вкладки
  const saveCurrentTab = async () => {
    const state = currentStateRef.current
    if (!state.activeTab || isLoadingTabRef.current) return
    ...
    try {
      ...
      await updatePivotReport(state.activeTab.id, {
        ...
        pivotData: state.pivotData || undefined,
      })
      hasUnsavedChangesRef.current = false
      console.log('✅ Вкладка сохранена')
    } catch (err: any) {
  ...
  // Оптимизированная загрузка: UI показывается сразу, данные подгружаются асинхронно
  useEffect(() => {
    ...
    const loadAllData = async () => {
      try {
        ...
        console.log('🚀 Начало загрузки данных для pivot...')
        
        // Сначала показываем UI (не блокируем его загрузкой)
        // Загружаем данные в фоне
        setLoadingFields(true)
        setLoadingParameters(true)
        
        // Загружаем названия проекта/версии в фоне (не блокируют UI)
        Promise.all([
          getProject(projectId).catch((err) => {
            console.warn('⚠️ Ошибка загрузки названия проекта (не критично):', err)
            return { name: '' }
          }),
          getProjectVersion(projectId, versionId).catch((err) => {
            console.warn('⚠️ Ошибка загрузки названия версии (не критично):', err)
            return { name: '' }
          })
        ...
        // Загружаем поля и параметры параллельно (критичные данные)
        const [fieldsResponse, parameters] = await Promise.all([
          getAvailableFields({
            projectId,
            versionId,
          }),
          getAvailableParameters({
            ...
            console.warn('⚠️ Ошибка загрузки параметров (не критично):', err)
        ...
        // Устанавливаем поля и параметры
        setAvailableFields(fieldsResponse.fields)
        setAvailableParameters(parameters)
        
        console.log('✅ Загружены данные для pivot:', {
          fields: fieldsResponse.fields.length,
  ...
  // Сохранение при обновлении/закрытии страницы
  useEffect(() => {
    const handleBeforeUnload = () => {
      const state = currentStateRef.current
      if (hasUnsavedChangesRef.current && state.activeTab) {
        // Используем fetch с keepalive для сохранения перед закрытием
        const cleanedFiltersForSave: Record<string, string[]> = {}
        Object.entries(state.filters).forEach(([field, values]) => {
          if (field !== 'selected_parameters' && !field.includes('selected_parameters')) {
            cleanedFiltersForSave[field] = values
        ...
        const requestBody = JSON.stringify({
          selected_parameters: state.selectedParameters.length > 0 ? state.selectedParameters : undefined,
          rows: state.rows,
          columns: state.columns,
          ...
          filters: Object.keys(cleanedFiltersForSave).length > 0 ? cleanedFiltersForSave : undefined,
          pivot_data: state.pivotData ? {
            rows: state.pivotData.rows || [],
            columns: state.pivotData.columns || [],
            cells: (state.pivotData.cells || []).map(cell => ({
              row_key: cell.rowKey,
              column_key: cell.columnKey,
              values: cell.values
            })),
            aggregations: (state.pivotData.aggregations || []).map(agg => ({
              field: agg.field,
              function: agg.function,
              display_name: agg.displayName
            })),
            total_rows: state.pivotData.totalRows || 0,
            ...('rowsFields' in state.pivotData && state.pivotData.rowsFields ? { rows_fields: state.pivotData.rowsFields } : {}),
            ...('columnsFields' in state.pivotData && state.pivotData.columnsFields ? { columns_fields: state.pivotData.columnsFields } : {})
          } : undefined,
        })
        
        // Используем fetch с keepalive для сохранения перед закрытием
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/datalab'
        const baseUrl = apiUrl.includes('localhost') || apiUrl.includes('127.0.0.1') 
          ? '/api/datalab' 
          : apiUrl
        const url = `${baseUrl}/pivot/reports/${state.activeTab.id}`
        
        // Пытаемся сохранить синхронно (может не сработать в некоторых браузерах)
        fetch(url, {
          method: 'PUT',
          headers: {
  ...
  const handleTabChange = async (tab: PivotReport) => {
    // Если переключаемся на ту же вкладку, ничего не делаем
    if (activeTab?.id === tab.id) {
      console.log('⚠️ Попытка переключиться на ту же вкладку, пропускаем')
      return
    }
    
    // ВСЕГДА сохраняем текущую вкладку перед переключением
    // Это гарантирует, что все настройки и таблица будут сохранены
    if (activeTab) {
      try {
        console.log('💾 Сохранение текущей вкладки перед переключением...', {
          tabId: activeTab.id,
        ...
        // Принудительно сохраняем все данные текущей вкладки
        const savedTab = await updatePivotReport(activeTab.id, {
          selectedParameters,
          rows,
          columns,
          values,
        ...
        hasUnsavedChangesRef.current = false
        console.log('✅ Текущая вкладка сохранена перед переключением:', {
          savedTabId: savedTab.id,
          savedRows: savedTab.rows?.length || 0,
          savedColumns: savedTab.columns?.length || 0,
          savedFilters: Object.keys(savedTab.filters || {}).length
        })
      } catch (err: any) {
        console.error('❌ Ошибка сохранения перед переключением:', err)
        // Продолжаем переключение даже при ошибке сохранения, но предупреждаем пользователя
        alert('Не удалось сохранить настройки текущей вкладки. Они могут быть потеряны.')
      }
    }
    
    // Загружаем данные новой вкладки
    console.log('📂 Загрузка данных вкладки:', tab.name, tab.id)
    isLoadingTabRef.current = true
    
    try {
      // Загружаем свежие данные вкладки с сервера, чтобы убедиться, что у нас актуальная версия
      // Это гарантирует, что мы получим все сохраненные данные
      console.log('📡 Загрузка свежих данных вкладки с сервера...')
      const freshTab = await getPivotReport(tab.id)
      console.log('📡 Получены свежие данные вкладки:', {
        id: freshTab.id,
        name: freshTab.name,
        rows: freshTab.rows?.length || 0,
        columns: freshTab.columns?.length || 0,
        values: freshTab.values?.length || 0,
        filters: Object.keys(freshTab.filters || {}).length,
        hasPivotData: !!freshTab.pivotData
      })
      
      // Устанавливаем активную вкладку (используем свежие данные)
      setActiveTab(freshTab)
      setActiveTabId(freshTab.id)
      
      // Восстанавливаем все настройки из свежих данных вкладки
      const restoredSelectedParameters = freshTab.selectedParameters || []
      const restoredRows = freshTab.rows || []
      const restoredColumns = freshTab.columns || []
      const restoredValues = freshTab.values || []
      
      setSelectedParameters(restoredSelectedParameters)
      setRows(restoredRows)
      setColumns(restoredColumns)
      setValues(restoredValues)
      
      // Восстанавливаем фильтры
      const loadedFilters = freshTab.filters || {}
      ...
      const restoredPivotData = freshTab.pivotData || null
      setPivotData(restoredPivotData)
      setPivotError(null)
      
      console.log('✅ Данные вкладки восстановлены из сервера:', {
        selectedParameters: restoredSelectedParameters.length,
        rows: restoredRows.length,
        columns: restoredColumns.length,
        values: restoredValues.length,
        filters: Object.keys(cleanedFilters).length,
        hasPivotData: !!restoredPivotData,
        pivotDataRows: restoredPivotData?.rows?.length || 0,
        pivotDataCells: restoredPivotData?.cells?.length || 0
      })
    } catch (err: any) {
      console.error('❌ Ошибка загрузки данных вкладки с сервера, используем данные из кэша:', err)
      // Если не удалось загрузить с сервера, используем данные из tab (кэш)
      setActiveTab(tab)
      ...
      setPivotError('Не удалось загрузить свежие данные вкладки, используются данные из кэша')
    } finally {

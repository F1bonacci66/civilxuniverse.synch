#!/bin/bash
# Проверка основного nginx.conf и финальное исправление

echo "=========================================="
echo "ПРОВЕРКА ОСНОВНОГО NGINX.CONF И ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ"
echo "=========================================="
echo ""

echo "ШАГ 1: Проверка основного nginx.conf"
echo "--------------------------------------------------------"
echo "Проверяем, есть ли настройки, которые могут влиять на редирект:"
grep -E "merge_slashes|absolute_redirect|server_tokens" /etc/nginx/nginx.conf
echo ""

echo "ШАГ 2: Проверка всех конфигов nginx"
echo "--------------------------------------------------------"
echo "Проверяем, есть ли другие конфиги, которые могут влиять:"
ls -la /etc/nginx/conf.d/
echo ""

echo "ШАГ 3: Проверка, может быть проблема в том, что nginx считает путь директорией"
echo "--------------------------------------------------------"
echo "Проблема может быть в том, что nginx автоматически добавляет trailing slash для 'директорий'"
echo ""

echo "ШАГ 4: Применение финального исправления"
echo "--------------------------------------------------------"
echo "Используем подход с обработкой OPTIONS через отдельный location блок"
echo "с максимальным приоритетом (^~) для префикса /api/datalab/auth/register"
echo ""

# Создаем конфигурацию с обработкой OPTIONS через location блок с максимальным приоритетом
...
if [ $? -eq 0 ]; then
    ...
    if [ "$STATUS" = "204" ]; then
        ...
        echo ""
        echo "ВОЗМОЖНАЯ ПРИЧИНА: Редирект происходит на уровне FastAPI или в другом месте."
        echo "Попробуйте проверить, может быть нужно перезапустить FastAPI."
    fi
else

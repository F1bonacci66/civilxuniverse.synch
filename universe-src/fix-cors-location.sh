#!/bin/bash
# Проверка заголовка Location и исправление

echo "🔍 Проверка заголовка Location..."
echo ""

echo "1. Куда идет редирект 308:"
...
echo "2. Проверка, может быть FastAPI делает редирект:"
...
echo "3. Проверка, может быть проблема в proxy_redirect:"
echo ""

echo "🔧 Применение исправления - отключаем proxy_redirect для OPTIONS..."

# Создаем конфигурацию с отключенным proxy_redirect и правильной обработкой OPTIONS
...
server {
    ...
    location = /api/datalab/auth/register {
        # Обрабатываем OPTIONS запросы БЕЗ proxy_pass
        if ($request_method = OPTIONS) {
            add_header Access-Control-Allow-Origin "*" always;
            add_header Access-Control-Allow-Methods "GET, POST, PUT, DELETE, PATCH, OPTIONS" always;
            add_header Access-Control-Allow-Headers "Authorization, Content-Type, X-Requested-With" always;

#!/bin/bash
# ИСПРАВЛЕНИЕ: Использование map для обработки OPTIONS на уровне server

echo "=========================================="
echo "ИСПРАВЛЕНИЕ CORS 308: MAP ДЛЯ OPTIONS"
echo "=========================================="
echo ""

# Проверяем, есть ли map в основном nginx.conf
if ! grep -q "map.*request_method.*OPTIONS" /etc/nginx/nginx.conf; then
    echo "Добавляем map в основной nginx.conf..."
    # Создаем резервную копию
    cp /etc/nginx/nginx.conf /etc/nginx/nginx.conf.backup.$(date +%Y%m%d_%H%M%S)
    
    # Добавляем map в http блок
    sed -i '/^http {/a\    map $request_method $is_options {\n        default 0;\n        OPTIONS 1;\n    }' /etc/nginx/nginx.conf
    echo "✅ Map добавлен в nginx.conf"
fi

# Обновляем конфигурацию api.civilx.ru
...
server {
    ...
    # КРИТИЧНО: Используем ^~ для максимального приоритета
    # Это обрабатывается ПЕРВЫМ, до любых других location блоков

#!/bin/bash
# Проверка и исправление настроек FastAPI на сервере

echo "=========================================="
echo "ПРОВЕРКА И ИСПРАВЛЕНИЕ FASTAPI"
echo "=========================================="
echo ""

echo "ШАГ 1: Проверка настроек FastAPI на сервере"
echo "--------------------------------------------------------"
if [ -f /opt/civilx-backend/app/main.py ]; then
    echo "Файл найден: /opt/civilx-backend/app/main.py"
    echo ""
    echo "Проверяем настройку redirect_slashes:"
    grep -A 5 "FastAPI(" /opt/civilx-backend/app/main.py | grep -E "redirect_slashes|FastAPI"
    echo ""
    
    if ! grep -q "redirect_slashes=False" /opt/civilx-backend/app/main.py; then
        echo "❌ ПРОБЛЕМА: redirect_slashes=False отсутствует в FastAPI!"
        echo "Это может быть причиной редиректа 308."
        echo ""
        echo "Нужно обновить файл /opt/civilx-backend/app/main.py"
    else
        echo "✅ redirect_slashes=False найден в коде"
    fi
else
    echo "Файл не найден в /opt/civilx-backend/app/main.py"
    echo "Ищем main.py:"
    find /opt -name "main.py" -type f 2>/dev/null | head -3
fi
echo ""

echo "ШАГ 2: Проверка роутера"
echo "--------------------------------------------------------"
if [ -f /opt/civilx-backend/app/api/v1/__init__.py ]; then
    echo "Проверяем настройку роутера:"
    grep "APIRouter" /opt/civilx-backend/app/api/v1/__init__.py
else
    echo "Файл роутера не найден"
fi
echo ""

echo "ШАГ 3: Тест FastAPI напрямую"
echo "--------------------------------------------------------"
echo "Тест OPTIONS на /api/datalab/auth/register:"
...
echo "ШАГ 4: Применение исправления nginx (обработка OPTIONS БЕЗ proxy_pass)"
echo "--------------------------------------------------------"

# Создаем конфигурацию, которая обрабатывает OPTIONS БЕЗ proxy_pass
...
server {
    ...
    location = /api/datalab/auth/register {
        ...
        # Это предотвращает редирект от FastAPI
        if ($request_method = OPTIONS) {
            add_header Access-Control-Allow-Origin "*" always;
            add_header Access-Control-Allow-Methods "GET, POST, PUT, DELETE, PATCH, OPTIONS" always;
            add_header Access-Control-Allow-Headers "Authorization, Content-Type, X-Requested-With" always;
...
if [ $? -eq 0 ]; then
    ...
    if [ "$STATUS" = "204" ]; then
        ...
        echo ""
        echo "Проверка заголовка Location:"
        curl -v -X OPTIONS https://api.civilx.ru/api/datalab/auth/register \
          -H "Origin: http://civilxuniverse.ru" \
          -H "Access-Control-Request-Method: POST" \
          -k 2>&1 | grep -E "^< HTTP|^< Location"
        echo ""
        echo "РЕКОМЕНДАЦИЯ: Проверьте настройки FastAPI на сервере."
        echo "Убедитесь, что в /opt/civilx-backend/app/main.py есть:"

#!/bin/bash
echo "=========================================="
echo "ПОЛНАЯ НАСТРОЙКА API НА civilxuniverse.ru"
echo "=========================================="
echo ""
...
server {
    ...
    # Используем location = для точного совпадения (максимальный приоритет)
...
STATUS2=$(curl -s -o /dev/null -w "%{http_code}" -X OPTIONS http://127.0.0.1/api/datalab/auth/register -H "Host: civilxuniverse.ru" -H "Origin: http://civilxuniverse.ru" -H "Access-Control-Request-Method: POST")

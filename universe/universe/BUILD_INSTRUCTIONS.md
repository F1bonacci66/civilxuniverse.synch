# 🐳 Инструкция по сборке Docker образа

## Предварительные требования

1. **Docker Desktop установлен и запущен**
   - Скачать: https://www.docker.com/products/docker-desktop/
   - После установки перезапустите компьютер
   - Убедитесь, что Docker Desktop запущен (иконка в системном трее)

2. **Проверка Docker:**
   ```powershell
   docker --version
   docker ps
   ```

## Сборка образа

### Вариант 1: Использовать скрипт (рекомендуется)

**Windows PowerShell:**
```powershell
cd C:\Projects\CivilX\Site\civilx-website\universe\universe
.\build-docker.ps1
```

Скрипт автоматически:
- Проверит наличие Docker
- Проверит, что Docker Desktop запущен
- Включит BuildKit
- Соберёт образ с тегами `civilx-universe:latest` и `civilx-universe:v1.0.0`

### Вариант 2: Собрать вручную

**Windows PowerShell:**
```powershell
cd C:\Projects\CivilX\Site\civilx-website\universe\universe

# Включить BuildKit для оптимизации сборки
$env:DOCKER_BUILDKIT=1
$env:COMPOSE_DOCKER_CLI_BUILD=1

# Собрать образ
docker build -t civilx-universe:latest -t civilx-universe:v1.0.0 .
```

**Linux/Mac:**
```bash
cd /path/to/civilx-website/universe/universe

# Включить BuildKit
export DOCKER_BUILDKIT=1
export COMPOSE_DOCKER_CLI_BUILD=1

# Собрать образ
docker build -t civilx-universe:latest -t civilx-universe:v1.0.0 .
```

## Время сборки

- **Первая сборка**: 5-10 минут (загрузка базового образа, установка зависимостей)
- **Последующие сборки**: 1-3 минуты (используется кэш Docker)

## Проверка результата

После успешной сборки:

```powershell
# Просмотр созданных образов
docker images civilx-universe

# Должны быть видны:
# civilx-universe   latest    <image-id>   <time>   ~200-300MB
# civilx-universe   v1.0.0    <image-id>   <time>   ~200-300MB
```

## Тестирование образа локально

Перед деплоем на сервер рекомендуется протестировать образ:

```powershell
# Запустить контейнер для тестирования
docker run -d `
  --name test-universe `
  -p 3001:3001 `
  -e NEXT_PUBLIC_API_URL=https://api.civilx.ru/api/datalab `
  civilx-universe:latest

# Проверить логи
docker logs -f test-universe

# В другом терминале проверить health endpoint
curl http://localhost:3001/api/health
# Должен вернуть: {"status":"healthy","service":"civilx-universe",...}

# Остановить и удалить тестовый контейнер
docker stop test-universe
docker rm test-universe
```

## Сохранение образа для переноса на сервер

### Сохранение в tar файл

```powershell
# Сохранить образ в архив
docker save -o civilx-universe-latest.tar civilx-universe:latest

# Проверить размер файла
Get-Item civilx-universe-latest.tar | Select-Object Name, @{Name="Size(MB)";Expression={[math]::Round($_.Length/1MB,2)}}
```

Размер файла будет примерно 200-300 МБ.

### Сжатие архива (опционально)

```powershell
# Использовать 7-Zip или другой архиватор
# 7z a -tgzip civilx-universe-latest.tar.gz civilx-universe-latest.tar
```

## Устранение проблем

### Ошибка: "Docker Desktop не запущен"

**Решение:**
1. Запустите Docker Desktop из меню Пуск
2. Дождитесь полной загрузки (иконка в трее перестанет мигать)
3. Повторите сборку

### Ошибка: "request returned 500 Internal Server Error"

**Решение:**
1. Перезапустите Docker Desktop
2. Проверьте, что нет конфликтов портов
3. Убедитесь, что достаточно места на диске

### Ошибка при сборке: "npm ci failed"

**Решение:**
1. Убедитесь, что `package-lock.json` существует
2. Если нет, выполните локально: `npm install`
3. Повторите сборку

### Сборка занимает слишком много времени

**Решение:**
- Это нормально для первой сборки (5-10 минут)
- Последующие сборки будут быстрее благодаря кэшу
- Убедитесь, что BuildKit включен: `$env:DOCKER_BUILDKIT=1`

## Следующие шаги

После успешной сборки:

1. ✅ Образ собран и готов к деплою
2. 📦 Сохраните образ в tar файл (если нужно перенести на сервер)
3. 🚀 Следуйте инструкциям в `DEPLOY_DOCKER.md` для развёртывания на сервере

## Дополнительная информация

- Подробная документация по деплою: `DEPLOY_DOCKER.md`
- Быстрый старт с Docker: `DOCKER_QUICK_START.md`
- Полная документация по сборке: `DOCKER_BUILD.md`






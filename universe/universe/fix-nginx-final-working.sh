#!/bin/bash
# ФИНАЛЬНОЕ РАБОЧЕЕ РЕШЕНИЕ: Обработка OPTIONS ДО редиректа

echo "=========================================="
echo "ФИНАЛЬНОЕ РАБОЧЕЕ РЕШЕНИЕ"
echo "=========================================="
echo ""
...
server {
    ...
    # КРИТИЧНО: Обрабатываем OPTIONS на уровне server блока ДО редиректа
...
server {
    ...
    # КРИТИЧНО: Обрабатываем OPTIONS на уровне server блока ДО location блоков
    # Это предотвращает автоматическое добавление trailing slash
    if ($request_method = OPTIONS) {
        add_header Access-Control-Allow-Origin "*" always;
        add_header Access-Control-Allow-Methods "GET, POST, PUT, DELETE, PATCH, OPTIONS" always;
        add_header Access-Control-Allow-Headers "Authorization, Content-Type, X-Requested-With" always;

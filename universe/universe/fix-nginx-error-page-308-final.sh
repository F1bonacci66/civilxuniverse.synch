#!/bin/bash
# ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ: error_page для перехвата редиректов 308

echo "=========================================="
echo "ИСПРАВЛЕНИЕ CORS 308: ERROR_PAGE 308"
echo "=========================================="
echo ""
...
server {
    ...
    # Internal location для обработки OPTIONS
    ...
    # КРИТИЧНО: Перехватываем редиректы 308 для OPTIONS запросов
    error_page 308 @handle_options_redirect;
    
    location @handle_options_redirect {
        # Если это OPTIONS запрос, возвращаем 204
        if ($request_method = OPTIONS) {
            add_header Access-Control-Allow-Origin "*" always;
            add_header Access-Control-Allow-Methods "GET, POST, PUT, DELETE, PATCH, OPTIONS" always;
            add_header Access-Control-Allow-Headers "Authorization, Content-Type, X-Requested-With" always;
        ...
        # Для других методов - возвращаем редирект обратно
        return 308 $request_uri;
    }

    # Обработка /api/datalab/auth/register

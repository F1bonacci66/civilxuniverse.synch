#!/bin/bash
# Исправление: правильный порядок location блоков

echo "🔍 ПРОБЛЕМА: Регулярное выражение location ~ перехватывает запрос раньше точного совпадения"
echo "🔧 РЕШЕНИЕ: Перемещаем точные совпадения ПЕРЕД регулярными выражениями"
echo ""
...
server {
    ...
    # Обрабатываем OPTIONS запросы ДО редиректа на HTTPS
...
server {
    ...
    # КРИТИЧНО: Точные совпадения ДОЛЖНЫ быть ПЕРЕД регулярными выражениями
    # Nginx обрабатывает location блоки в следующем порядке:
    # 1. Точные совпадения (=) - наивысший приоритет
    # 2. Префиксы с ^~ - второй приоритет
    # 3. Регулярные выражения (~, ~*) - третий приоритет
    # 4. Префиксы - самый низкий приоритет
    ...
    # Auth endpoints - точные совпадения
    ...
    # Общий location для остальных API запросов - ПОСЛЕ всех точных совпадений
    location ~ ^/api/datalab/ {
        if ($request_method = OPTIONS) {
            add_header Access-Control-Allow-Origin "*" always;
            add_header Access-Control-Allow-Methods "GET, POST, PUT, DELETE, PATCH, OPTIONS" always;

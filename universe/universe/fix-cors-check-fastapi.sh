#!/bin/bash
# Проверка настроек FastAPI и исправление проблемы

echo "=========================================="
echo "ПРОВЕРКА FASTAPI И ИСПРАВЛЕНИЕ CORS"
echo "=========================================="
echo ""

echo "ШАГ 1: Проверка настроек FastAPI"
echo "--------------------------------------------------------"
if [ -f /opt/civilx-backend/app/main.py ]; then
    ...
    grep -A 5 "redirect_slashes" /opt/civilx-backend/app/main.py
    echo ""
    echo "Проверяем настройку FastAPI:"
    grep -A 10 "FastAPI(" /opt/civilx-backend/app/main.py | head -15
else
    echo "Файл не найден. Ищем main.py:"
    find /opt -name "main.py" -type f 2>/dev/null | head -3
fi
echo ""

echo "ШАГ 2: Проверка, делает ли FastAPI редирект"
echo "--------------------------------------------------------"
echo "Тест 1: OPTIONS на /api/datalab/auth/register (БЕЗ trailing slash):"
...
echo "Тест 2: OPTIONS на /api/datalab/auth/register/ (С trailing slash):"
curl -v -X OPTIONS http://127.0.0.1:8000/api/datalab/auth/register/ \
  -H "Origin: http://civilxuniverse.ru" \
  -H "Access-Control-Request-Method: POST" \
  2>&1 | grep -E "< HTTP|< Location"
...
echo "ШАГ 3: Проверка, может быть проблема в том, что nginx делает редирект из-за proxy_pass"
echo "--------------------------------------------------------"
echo "Проблема: редирект происходит ДО обработки location блока"
echo "Это может быть из-за того, что FastAPI делает редирект, и nginx его перехватывает"
echo ""

echo "ШАГ 4: Применение исправления - обрабатываем OPTIONS БЕЗ proxy_pass"
echo "--------------------------------------------------------"
...
server {
    ...
    location = /api/datalab/auth/register {
        ...
        # Для остальных методов - проксируем
        # КРИТИЧНО: Используем полный путь БЕЗ trailing slash
...
if [ $? -eq 0 ]; then
    ...
    if [ "$STATUS" = "204" ]; then
        ...
        echo ""
        echo "Возможно, нужно проверить настройки FastAPI на сервере."
    fi
else

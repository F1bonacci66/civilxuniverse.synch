#!/bin/bash
echo "=========================================="
echo "ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ NGINX ДЛЯ /signup/"
echo "=========================================="
echo ""
...
server {
    ...
    # КРИТИЧНО: location = имеет максимальный приоритет
    # Используем proxy_pass БЕЗ URI части, чтобы передать полный путь
    location = /api/datalab/auth/signup/ {
        add_header X-Location-Matched "exact-signup-slash" always;
        ...
        # БЕЗ URI части - передаем полный путь /api/datalab/auth/signup/
    ...
    location = /api/datalab/auth/signup {
        add_header X-Location-Matched "exact-signup" always;
    ...
    # Общий location для остальных /api/datalab/ запросов
...
if [ "$STATUS" = "204" ]; then
    ...
    curl -s -X OPTIONS https://api.civilx.ru/api/datalab/auth/signup/ \
      ...
      -k -I | grep -i "access-control\|X-Location"
else
    echo "❌ Статус: $STATUS"
    echo ""
    ...
    curl -v -X OPTIONS https://api.civilx.ru/api/datalab/auth/signup/ \
      -H "Origin: http://civilxuniverse.ru" \
      -H "Access-Control-Request-Method: POST" \
      -k 2>&1 | grep -E "^< HTTP|^< X-Location|^> OPTIONS"
fi

#!/bin/bash
# ИСПРАВЛЕНИЕ: Обработка OPTIONS напрямую БЕЗ proxy_pass

echo "=========================================="
echo "ИСПРАВЛЕНИЕ CORS 308: ПРЯМАЯ ОБРАБОТКА OPTIONS"
echo "=========================================="
echo ""

echo "ПРОБЛЕМА: Редирект происходит ДО location блока"
echo "РЕШЕНИЕ: Обрабатываем OPTIONS напрямую в nginx БЕЗ proxy_pass"
echo ""
...
server {
    ...
    # КРИТИЧНО: Обработка OPTIONS для register БЕЗ proxy_pass
    # Используем точное совпадение (=) - наивысший приоритет
    # И обрабатываем OPTIONS напрямую в nginx
    location = /api/datalab/auth/register {
        # Обрабатываем OPTIONS запросы ПРЯМО В NGINX БЕЗ proxy_pass
        if ($request_method = OPTIONS) {
            add_header Access-Control-Allow-Origin "*" always;
            add_header Access-Control-Allow-Methods "GET, POST, PUT, DELETE, PATCH, OPTIONS" always;
            add_header Access-Control-Allow-Headers "Authorization, Content-Type, X-Requested-With" always;
...
if [ $? -eq 0 ]; then
    ...
    if [ "$STATUS" = "204" ]; then
        ...
        echo ""
        echo "Проблема: Редирект происходит ДО location блока."
        echo "Попробуем другой подход - использовать limit_except или проверить,"
        echo "может быть проблема в том, что nginx считает путь директорией."
    fi
else

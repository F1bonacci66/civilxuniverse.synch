#!/bin/bash
# Скрипт для применения исправления CORS на сервере
# Выполнить на сервере: bash apply-cors-fix-on-server.sh
...
if [ $? -eq 0 ]; then
    ...
    if [ "$STATUS" = "204" ]; then
        ...
        echo ""
        echo "✅ Исправление применено успешно! Теперь регистрация должна работать без ошибок CORS."
    else
        echo "❌ Проблема! Статус код: $STATUS (ожидался 204)"
        echo "Проверьте логи: tail -f /var/log/nginx/api.civilx.ru.error.log"
    fi
else
    echo "❌ Ошибка в конфигурации Nginx"
    echo "Восстановление из резервной копии..."
    cp /etc/nginx/sites-available/api.civilx.ru.backup.* /etc/nginx/sites-available/api.civilx.ru 2>/dev/null
    exit 1
fi

#!/bin/bash
# ПОСЛЕДНЯЯ ПОПЫТКА: Использование named location для обработки OPTIONS

echo "=========================================="
echo "ПОСЛЕДНЯЯ ПОПЫТКА: Named location для OPTIONS"
echo "=========================================="
echo ""
...
server {
    ...
    # Named location для обработки OPTIONS - имеет максимальный приоритет
    ...
    location = /api/datalab/auth/register {
        # Проверяем OPTIONS ПЕРВЫМ ДЕЛОМ
        if ($request_method = OPTIONS) {
            return 204;
            add_header Access-Control-Allow-Origin "*" always;
            add_header Access-Control-Allow-Methods "GET, POST, PUT, DELETE, PATCH, OPTIONS" always;
    ...
    # Точное совпадение С trailing slash
    location = /api/datalab/auth/register/ {
        if ($request_method = OPTIONS) {
            return 204;
            add_header Access-Control-Allow-Origin "*" always;
...
if [ "$STATUS" = "204" ]; then
    ...
    echo "❌ Статус: $STATUS"
    echo "Проверяю, может быть проблема в том, что редирект происходит из-за того, что FastAPI делает редирект..."
    echo "Проверка напрямую к FastAPI:"
    curl -s -o /dev/null -w "FastAPI Status: %{http_code}\n" -X OPTIONS http://127.0.0.1:8000/api/datalab/auth/register -H "Origin: http://civilxuniverse.ru" -H "Access-Control-Request-Method: POST"
fi

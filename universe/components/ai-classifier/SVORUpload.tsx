...
export function SVORUpload({ onUploadComplete }: SVORUploadProps) {
  ...
  const parseCSV = (text: string): ParseResult => {
    ...
    const groupIndex = headers.findIndex(h => h.toLowerCase() === 'svor_group')
    const activeIndex = headers.findIndex(h => h.toLowerCase() === 'is_active')
    
    console.log('[SVOR Parse] Опциональные колонки:', {
      svor_description: descIndex,
      svor_group: groupIndex,
      is_active: activeIndex,
    })
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (groupIndex !== -1 && values[groupIndex]) {
        position.svorGroup = values[groupIndex].replace(/^"|"$/g, '').trim()
      }
      
      if (activeIndex !== -1 && values[activeIndex]) {
        const activeValue = values[activeIndex].replace(/^"|"$/g, '').trim().toLowerCase()
        position.isActive = activeValue === 'true' || activeValue === '1' || activeValue === 'да'
      }
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = await uploadSVORPositions(parseResult.positions)
  ...
  const downloadTemplate = () => {
    const template = `svor_code,svor_name
01.01.001,Стена кирпичная
01.01.002,Стена бетонная
01.02.001,Перегородка гипсокартонная
02.01.001,Окно пластиковое
03.01.001,Дверь металлическая`

...
# По умолчанию используем относительный путь для dev сервера (проксируется через nginx)
# Для production можно переопределить: --build-arg NEXT_PUBLIC_API_URL=https://api.civilx.ru/api/datalab
ARG NEXT_PUBLIC_API_URL=/api/datalab

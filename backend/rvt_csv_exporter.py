...
class RVTCSVExporterService:
    """Обертка над внешним RvtExporterCfg1.exe."""
    ...
    def convert(self, rvt_file_path: str, output_dir: str, log_callback=None) -> dict:
        """
        ...
        if not self.is_windows:
            ...
            # Включаем детальную отладку Wine для диагностики проблем
            # Можно переключить на "-all" для продакшена после решения проблем
            env.setdefault("WINEDEBUG", "err+all,warn+all,winediag")
            env.setdefault("WINEPREFIX", str(self.wine_prefix))
            # Устанавливаем локаль для корректной работы с путями
            env.setdefault("LC_ALL", "en_US.UTF-8")
            env.setdefault("LANG", "en_US.UTF-8")
            
            # ВАЖНО: Попытка исправить проблемы с доступом к файлам через Wine
            # Переопределяем msvcrt на native для лучшей совместимости с файловыми операциями
            # Это может помочь с ошибками _wsopen_dispatch
            # ПЕРЕД использованием native версии необходимо установить vcrun6 через winetricks
            ...
            print(f"      WINEDLLOVERRIDES: {env.get('WINEDLLOVERRIDES')}")
            print(f"      WINEDEBUG: {env.get('WINEDEBUG')}")
            print(f"      ⚠️ ВАЖНО: Используется msvcrt=native, требуется установка vcrun6 через winetricks")
    ...
    def _build_command(self, rvt_path_str: str, process_cwd: str = None) -> tuple:
        ...
        # Пример: RvtExporterCfg1.exe "C:\Projects\Демонстрация\Паркинг\тест\SOB_GLP_PD_K2_KR_2022.rvt"
        # КРИТИЧЕСКИ ВАЖНО: Всегда используем абсолютный путь через Z: драйв для Wine
        # Это может помочь избежать проблем с PATH_NOT_FOUND (error 3)
        
        # ВСЕГДА используем абсолютный путь через Z: драйв, даже если файл в process_cwd
        # Это может помочь Wine правильно найти файл и избежать ошибок _wsopen_dispatch
        wine_path = full_path.replace('/', '\\').lstrip('\\')
        if not wine_path.startswith('Z:'):
            wine_path = 'Z:\\' + wine_path
        
        print(f"   ✅ Используем абсолютный путь через Z: драйв (всегда): {wine_path}")
        print(f"   ℹ️ Это может помочь избежать ошибок PATH_NOT_FOUND в Wine")
        
        # Дополнительная проверка: убеждаемся, что путь правильно экранирован для Wine
        # Заменяем обратные слэши на двойные для Windows-стиля пути
        wine_path = wine_path.replace('\\', '\\\\')
        print(f"   ✅ Путь экранирован для Wine: {wine_path}")
        ...
        # Используем одинарные кавычки для shell, внутри них двойные для экспортёра
        # Экранируем специальные символы в пути для двойных кавычек
    ...
    def _ensure_wine_prefix(self) -> None:
        ...
        # ВАЖНО: Устанавливаем vcrun6 через winetricks для поддержки msvcrt=native
